...
let BtnGuardarVenta = document.getElementById("BtnGuardarVenta");
...
fetch('http://localhost:5000/consultar_ventasAutolavados', settings)
    ...
    .then(response => {
        ...
        response.data.forEach(producto => {
            ...
            row.innerHTML = `
                <td>${producto.id_ventasAutolavado}</td>
...
BtnGuardarVenta.addEventListener("click", (e) => {
    const selectedRowVentas = getSelectedRowVentas();

    if (selectedRowVentas) {
        let dataFormat = {
            id_ventasAutolavado:selectedRowVentas.cells[0].innerText,
        ...
        fetch('http://localhost:5000/modificando_ventaAutolavado', settings)
            .then(rest => rest.ok ? rest.json() : Promise.reject(rest))
            .then(json => {
                console.log("Se conecto con el Servidor", json);
                location.href="./../Vistas/VentasAutolavado.html"
            })
            .catch(error => {
        ...
        fetch('http://localhost:5000/agregando_ventaAutolavado', settings)
            .then(rest => rest.ok ? rest.json() : Promise.reject(rest))
            .then(json => {
                console.log("Se conecto con el Servidor", json);
                location.href="./../Vistas/VentasAutolavado.html"
...
eliminarVentasBtn.addEventListener('click', function () {
    ...
    if (selectedRow) {
      ...
      confirmBtn.addEventListener('click', function () {
        ...
        fetch(`http://localhost:5000/eliminando_ventaAutolavado/${idInventario}`, settings)
          .then(rest => rest.ok ? rest.json() : Promise.reject(rest))
          .then(json => {
...
consultarVentasBtn.addEventListener('click', function () {
    ...
    confirmButton.addEventListener('click', function () {
      ...
      if (productRow) {
        ...
        const costo = productRow.cells[4].innerText;
  ...
  modificarVentaBtn.addEventListener('click', function () {
    const selectedRowVentas = getSelectedRowVentas();
    if (selectedRowVentas) {
      const id_ventasAutolavado = selectedRowVentas.cells[0].innerText;
      const id_usuario = selectedRowVentas.cells[1].innerText;
      const fecha = selectedRowVentas.cells[2].innerText;
      const tipoVehiculo = selectedRowVentas.cells[3].innerText;
      const costo = selectedRowVentas.cells[4].innerText;
